    ...
    fn block_on_external<T: Send>(
        ...
        if tokio::runtime::Handle::try_current().is_ok() {
            std::thread::scope(|scope| {
                scope
                    .spawn(|| f(self.rt.as_ref().unwrap()))
                    .join()
                    .map_err(|_| anyhow::anyhow!("daemon runtime thread panicked"))?
            })
        } else {
            f(self.rt.as_ref().unwrap())
        }
    }
...
impl ExecutionService for RemoteExecutionService {
    ...
    fn execute_with_start(
        ...
        let mut stream = self
            .block_on_external(|rt| Ok(rt.block_on(c.execute(Request::new(req)))?.into_inner()))?;
        loop {
                    ...
                    Some(proto::execute_event::Event::Finished(f)) => {
                        return match f.result {
                            Some(proto::finished::Result::Outcome(o)) => {
                                Ok(convert::outcome_from_proto(o))
                            }
                            Some(proto::finished::Result::Failure(f)) => bail!("{}", f.message),
                            None => bail!("daemon returned empty result"),
                        }
                    }
                    None => {}
                },
                None => bail!("daemon closed execution stream"),
            }
        }
    }
    fn cache_dir_get(&self, w: &str, n: &str, k: &str) -> Result<Option<PathBuf>> {
        let mut c = self.client()?;
        let x = self.block_on_external(|rt| {
            Ok(rt
                .block_on(c.cache_dir_get(Request::new(proto::CacheDirRequest {
                    workspace_id: w.into(),
                    name: n.into(),
                    key: k.into(),
                    source: String::new(),
        ...
        if !x.path.is_empty() {
            return Ok(Some(x.path.into()));
        }
        let path = imp_store::cache::named_cache_key_path_by_id(w, n, k)?;
        if path.is_dir() {
            Ok(Some(path))
        } else {
            bail!("daemon returned a present cache entry without a path")
        }
    }
    ...
    fn cache_dir_put(&self, w: &str, n: &str, k: &str, s: &Path) -> Result<()> {
        let mut c = self.client()?;
        self.block_on_external(|rt| {
            rt.block_on(c.cache_dir_put(Request::new(proto::CacheDirRequest {
                workspace_id: w.into(),
                name: n.into(),
                key: k.into(),
    ...
    fn extract_archive(&self, a: &Path, d: &Path, f: &str, s: u32) -> Result<()> {
        let mut c = self.client()?;
        self.block_on_external(|rt| {
            rt.block_on(c.extract_archive(Request::new(proto::ExtractRequest {
                archive: a.to_string_lossy().into_owned(),
                dest: d.to_string_lossy().into_owned(),
                format: f.into(),
                strip_components: s,
            })))?;
            Ok(())
    ...
    async fn worker_start(&self, w: &str, n: &str, s: WorkerSpec) -> Result<WorkerHandle> {
        ...
        let mut c = proto::execution_client::ExecutionClient::new(channel);
        let x = c
            .worker_start(Request::new(proto::WorkerRequest {
                workspace_id: w.into(),
                name: n.into(),
                argv: s.argv,
                env: s.env.into_iter().map(|(a, b)| format!("{a}={b}")).collect(),
                health_check_argv: s.health_check_argv,
            }))
            .await?
            .into_inner();
        Ok(WorkerHandle {
            home_dir: x.home_dir.into(),
            tmp_dir: x.tmp_dir.into(),
            port: x.port as u16,
        })
    }
    fn worker_get(&self, n: &str) -> Result<Option<WorkerHandle>> {
        let mut c = self.client()?;
        let x = self.block_on_external(|rt| {
            Ok(rt
                .block_on(c.worker_get(Request::new(proto::WorkerRequest {
                    name: n.into(),
                    ..Default::default()
                })))?
                .into_inner())
        })?;
        Ok(x.present.then(|| WorkerHandle {
            home_dir: x.home_dir.into(),
            tmp_dir: x.tmp_dir.into(),
            port: x.port as u16,
        }))
    ...
    fn capabilities(&self) -> Result<Capabilities> {
        ...
        let x = self.block_on_external(|rt| {
            Ok(rt
                .block_on(c.get_capabilities(Request::new(proto::Empty {})))?
                .into_inner())
        })?;
        Ok(Capabilities {
            os: Box::leak(x.os.into_boxed_str()),
            arch: Box::leak(x.arch.into_boxed_str()),
        })
    }
}
