...
impl ExecutionService for RemoteExecutionService {
    ...
    fn cache_dir_get(&self,w:&str,n:&str,k:&str)->Result<Option<PathBuf>>{let mut c=self.client()?;let x=self.block_on_external(|rt| Ok(rt.block_on(c.cache_dir_get(Request::new(proto::CacheDirRequest{workspace_id:w.into(),name:n.into(),key:k.into(),source:String::new()})))?.into_inner()))?;if !x.present{return Ok(None)}if !x.path.is_empty(){return Ok(Some(x.path.into()))}let path=imp_store::cache::named_cache_key_path_by_id(w,n,k)?;if path.is_dir(){Ok(Some(path))}else{bail!("daemon returned a present cache entry without a path")}}
