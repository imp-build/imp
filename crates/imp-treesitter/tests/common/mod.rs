//! Path to the `tree-sitter-json` fixture grammar (see
//! `testdata/tree-sitter-json/README.md` for provenance).
//!
...
//! (`include_bytes!`) rather than read from disk at test-run time: imp's own
//! Rust-test execution runs a compiled test binary as its own sandboxed
//! action whose declared inputs are just that binary's build output digest —
//! never the source tree — so a path resolved at runtime doesn't resolve to
//! anything in that sandbox. Embedding the bytes makes the fixture part of
//! the binary itself, which the run action already has; we just need to
//! spill it to a real file on disk once, since `load_grammar` dlopens a
//! path.
