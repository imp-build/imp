...
/// Call a `//module/path:exportName` address directly with a single string
/// argument, bypassing the target/goal/selector system entirely — the Phase
/// 4 pilot (`module:export` addressing, #52). `module_path` is resolved and
/// `import()`ed exactly as `BUILD.js`/rule modules already are; `export_name`
/// is looked up on the resulting namespace and called with `arg`, awaiting a
/// promise result if one is returned. Errors are labeled `module_path:export_name`,
/// mirroring the `target.address#product` convention `execute_goal_live_selection`
/// uses for its own per-call labels.
pub async fn call_address_live(
    live: &LiveWorkspace,
    workspace_root: &Path,
    module_path: &str,
    export_name: &str,
    ...
    *live.exec_root.lock().unwrap() = Some(workspace_root.to_owned());
    ...
    let module_path_owned = module_path.to_owned();
    let export_name_owned = export_name.to_owned();
    ...
    let result = live
        ...
        .async_with(async move |ctx| -> rquickjs::Result<()> {
            ...
            let promise = Module::import(&ctx, module_path_owned.as_str())
                .catch(&ctx)
                .map_err(|e| {
                    rquickjs::Error::new_loading_message(
                        "execute",
                        format!("resolve module '{module_path_owned}': {e}"),
                    )
                })?;
            let ns: Object = promise.into_future().await.catch(&ctx).map_err(|e| {
                rquickjs::Error::new_loading_message(
                    "execute",
                    format!("resolve module '{module_path_owned}': {e}"),
                )
            })?;
            let export_fn: Function = ns.get(export_name_owned.as_str()).map_err(|_| {
                rquickjs::Error::new_loading_message(
                    "execute",
                    format!("'{export_name_owned}' is not exported by {module_path_owned}"),
                )
            })?;

            let promise_resolve: Function = ctx.eval("(value) => Promise.resolve(value)")?;
            let result_value: Value = export_fn.call((arg_owned,)).catch(&ctx).map_err(|e| {
                rquickjs::Error::new_loading_message("execute", format!("{label_for_ctx}: {e}"))
            })?;
