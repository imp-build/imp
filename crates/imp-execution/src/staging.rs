...
pub fn resolve_input_digest(workspace_root: &Path, inputs: &[ExecIoSpec]) -> Result<String> {
    ...
    for input in inputs {
        let tree = match input.kind.as_str() {
            "digest" => DirectoryDigest::from_digest(input.digest.clone().ok_or_else(|| anyhow::anyhow!("digest input is missing its digest"))?),
            "file" | "manifest" => {
                let path = input.path.as_ref().ok_or_else(|| anyhow::anyhow!("{} input is missing its path", input.kind))?;
            ...
            "directory" => {
                let path = input.path.as_ref().ok_or_else(|| anyhow::anyhow!("directory input is missing its path"))?;
                let captured = capture_directory(&workspace_root.join(artifact_relative_path(path)?))?;
...
pub fn materialize_outputs(outputs: &[imp_store::cache::CachedArtifact], workspace_root: &Path) -> Result<()> {
    materialize_cached_artifacts(outputs, workspace_root)
}

pub fn materialize_named_caches(outputs: &[imp_store::cache::CachedArtifact], workspace_id: &str) -> Result<()> {
