...
} from "//rules/rust/generate_build";
...
describe("rust generate-build", () => {
	...
	test("package_for_manifest matches the package whose manifest_path ends with the given path", () => {
		const metadata = {
			...
			packages: [
				{
					name: "imp",
					manifest_path: "/sandbox/repo/Cargo.toml",
					targets: [],
				},
				{
					name: "imp-store",
					manifest_path: "/sandbox/repo/crates/imp-store/Cargo.toml",
					targets: [],
				},
			],
		};
	...
	test("package_for_manifest returns null when no package matches", () => {
		...
		expect(package_for_manifest(metadata, "crates/missing/Cargo.toml")).toBe(
			null,
		);
	});
});
