...
describe("sccache toolchain", () => {
	test("declares a default sccache toolchain", () => {
		return withSccacheHost((host) => {
			...
			expect(
				sccacheCacheKey(toolchain.attrs.version, {
					os: "linux",
					arch: "x86_64",
				}),
			).toBe("0.10.0/linux-x86_64");
	...
	test("installs and acquires a toolchain from the named cache", async () => {
		await withSccacheHost(async (host) => {
			...
			expect(installSccacheToolchain("0.10.0", "/tmp/sccache-0.10.0")).toBe(
			...
			expect(
				host.calls.some(
					(call) =>
						call[0] === "cachePut" &&
						call[1] === "sccache-toolchains" &&
						call[2] === key &&
						call[3] === "/tmp/sccache-0.10.0",
				),
			).toBe(true);

			expect(await acquireSccacheToolchain("0.10.0")).toBe(
				"/cache/sccache-toolchains/0.10.0/linux-x86_64",
			);
	...
	test("downloads, verifies, and extracts a toolchain via two sandboxed runs", async () => {
		await withSccacheHost(async (host) => {
			const key = sccacheCacheKey("0.10.0", { os: "linux", arch: "x86_64" });
			host.addFile(
				"//rules/rust/sccache/sccache.lock",
				JSON.stringify({
					tool: "sccache",
					versions: {
						"0.10.0": {
			...
			expect(download.argv).toContain(
				"https://locked.example/sccache-v0.10.0-x86_64-unknown-linux-musl.tar.gz",
			);
