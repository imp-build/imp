...
// but declared explicitly here (like build/test/fmt/run) so its graph
// handler, graphPackageGoal, actually runs instead of Rust's generic
// per-target dispatch.
//
...
// docs/BUILD.js's site package handler does — attach(label, "package", fn)
// (the `packageGoal()` sugar in imp:core) is a separate, still-supported
// mechanism and is unaffected by this goal's own dispatch.
//
...
goal("package", undefined, { graph: graphPackageGoal });
