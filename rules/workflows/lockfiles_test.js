...
import { generateToolLockfile, generateBuiltinLockfile, builtinLockfiles } from "//rules/workflows/lockfiles";
...
test("merging preserves other locked versions, sorted", async () => {
    const existing = {
        tool: "tool",
        versions: {
            "2.0.0": { "linux/x86_64": { url: "u2", artifact: "a2", size: 2, sha256: "s2" } },
            "0.9.0": { "linux/x86_64": { url: "u0", artifact: "a0", size: 0, sha256: "s0" } },
        },
    };
...
test("re-locking an existing version replaces that version's entries", async () => {
    const existing = {
        ...
        versions: {
            "1.2.3": {
                "linux/x86_64": { url: "stale", artifact: "stale.tar", size: 1, sha256: "stale" },
                "macos/aarch64": { url: "gone", artifact: "gone.tar", size: 1, sha256: "gone" },
            },
        },
    };
    const host = fakeHost({ "//tool.lock": JSON.stringify(existing) });
...
test("existing lock for a different tool or invalid JSON is ignored", async () => {
    for (const contents of [JSON.stringify({ tool: "other", versions: { x: {} } }), "not json"]) {
...
test("writes to the lockfile address when one is given, merging from it", async () => {
    const existing = {
        tool: "widget",
        versions: { "8": { "linux/x86_64": { url: "u8", artifact: "a8", size: 8, sha256: "s8" } } },
    };
    const host = fakeHost({ "//rules/widgets/widget.lock": JSON.stringify(existing) });
    const handle = { attrs: { version: "9" } };
    const lock = await generateToolLockfile(
        {
            handle,
...
test("writes exactly the registered versions, ignoring existing contents", async () => {
    const existing = {
        tool: "tool",
        versions: {
            "0.1.0": { "linux/x86_64": { url: "stale", artifact: "stale.tar", size: 1, sha256: "stale" } },
        },
    };
...
test("every toolchain module registers its shipped lockfile", () => {
    const registered = builtinLockfiles().map((spec) => spec.name).sort();
    expect(registered).toEqual([
        "cmake", "crane", "gcc", "mold", "odin",
        "pex-toolchain", "ruff-toolchain", "rust", "sccache",
        "uv-toolchain", "zig", "zola",
    ]);
