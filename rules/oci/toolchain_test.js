...
describe("crane toolchain", () => {
	test("builds artifact name / download URL per platform", () => {
		...
		expect(craneCacheKey("0.20.6", { os: "linux", arch: "x86_64" })).toBe(
			"0.20.6/linux-x86_64",
		);
	});
	...
	test("installs and acquires a toolchain from the named cache", async () => {
		await withCraneHost(async (host) => {
			...
			expect(installCraneToolchain("0.20.6", "/tmp/crane-0.20.6")).toBe(
				"/cache/crane-toolchains/0.20.6/linux-x86_64",
			);
			expect(
				host.calls.some(
					(call) =>
						call[0] === "cachePut" &&
						call[1] === "crane-toolchains" &&
						call[2] === key &&
						call[3] === "/tmp/crane-0.20.6",
				),
			).toBe(true);

			expect(await acquireCraneToolchain("0.20.6")).toBe(
				"/cache/crane-toolchains/0.20.6/linux-x86_64",
			);
			expect(await craneBin("0.20.6")).toBe(
				"/cache/crane-toolchains/0.20.6/linux-x86_64/crane",
			);
	...
	test("downloads, verifies, and extracts a toolchain via two sandboxed runs", async () => {
		await withCraneHost(async (host) => {
			...
			expect(download.argv).toContain(
				"https://locked.example/go-containerregistry_Linux_x86_64.tar.gz",
			);
