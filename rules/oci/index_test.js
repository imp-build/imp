...
describe("oci rules", () => {
	...
	test("ociPull requires exactly one of tag/digest", () => {
		...
		expect(() =>
			ociPull({ repo: "alpine", tag: "latest", digest: DIGEST_A }),
		).toThrow("exactly one of 'tag' or 'digest'");
	});

	test("ociPull by tag resolves a digest (impure) then pulls it (cacheable)", async () => {
		await withOciHost(
			withCraneStdout([["crane digest", DIGEST_A]], async (host) => {
				const target = ociPull({
	...
	test("ociPull short-circuits when the digest is already cached", async () => {
		await withOciHost(async (host) => {
			host.install(
				"oci-storage",
				"sha256/" + "a".repeat(64),
				"/cache/oci-storage/sha256/" + "a".repeat(64),
			);
			const target = ociPull({
				repo: "docker.io/library/alpine",
				digest: DIGEST_A,
			...
			expect(result.ociLayoutPath).toBe(
				"/cache/oci-storage/sha256/" + "a".repeat(64),
			);
	...
	test("ociBuild requires at least one layer", () => {
		expect(() => ociBuild({ base: "scratch", layers: [] })).toThrow(
			"requires one or more 'layers'",
		);
	});
	...
	test("ociBuild stages deterministic layer tarballs and composes onto its base", async () => {
		await withOciHost(
			withCraneStdout([["oci-compose", DIGEST_B]], async (host) => {
				...
				expect(
					stageRun.argv.some(
						(arg) => typeof arg === "string" && arg.includes('--mtime="@0"'),
					),
				).toBe(true);
				expect(
					stageRun.argv.some(
	...
	test("ociPush requires image/repo/tag", () => {
		expect(() => ociPush({ repo: "ghcr.io/org/app", tag: "v1" })).toThrow(
			"requires 'image'",
		);
		expect(() =>
			ociPush({ image: ociPull({ repo: "alpine", tag: "latest" }), tag: "v1" }),
		).toThrow("requires 'repo'");
		expect(() =>
			ociPush({
				image: ociPull({ repo: "alpine", tag: "latest" }),
	...
	test("ociPush resolves its image's build product and pushes impurely", async () => {
		await withOciHost(async (host) => {
			...
			expect(pushRun.argv).toEqual([
				"crane",
				"push",
				".imp/tools/oci-push-image",
				"ghcr.io/org/app:v1",
			]);
	...
	test("ociMirror requires from/to", () => {
		expect(() =>
			ociMirror({ to: { repo: "ghcr.io/org/app", tag: "v1" } }),
		).toThrow("requires 'from");
		expect(() =>
			ociMirror({ from: { repo: "docker.io/library/alpine", tag: "3.20" } }),
		).toThrow("requires 'to");
	});
