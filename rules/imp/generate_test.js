        ...
        await withFakeDiff([], async () => {
            await withFakeWriteWorkspace(async () => {
                await generatedFiles({
                    ...
                    argv: ["gen", "--flag"],
                    tools: [{ name: "gen", cache: "x", key: "x", path: "/bin/gen", binDirs: ["."] }],
                    inputs: [],
                    outputPaths: [A, B],
                    materialize: true,
                });
                const { trace } = getMemoTrace();
                const runEffect = trace.find((t) => t.event === "effect" && t.kind === "run");
    ...
    await withFakeRun(async () => {
        await withFakeDiff([], async () => {
            await withFakeWriteWorkspace(async (calls) => {
                await generatedFiles({ argv: ["gen"], outputPaths: [A, B], materialize: true });
                expect(calls.length).toBe(2);
                expect(calls[0]).toEqual({ path: A, digest: calls[0].digest, from: A });
                expect(calls[1]).toEqual({ path: B, digest: calls[1].digest, from: B });
            });
        });
    });
});
...
test("materialize: false never calls writeWorkspace", async () => {
    await withFakeRun(async () => {
        await withFakeDiff([{ type: "modified", path: A }], async () => {
            await withFakeWriteWorkspace(async (calls) => {
                await generatedFiles({ argv: ["gen"], outputPaths: [A], materialize: false });
...
test("changed is empty when the diff reports no changes", async () => {
    await withFakeRun(async () => {
        await withFakeDiff([], async () => {
            const { changed } = await generatedFiles({ argv: ["gen"], outputPaths: [A, B], materialize: false });
...
test("an ancestor-directory diff entry (whole new subtree) marks every output under it as changed", async () => {
    ...
    await withFakeRun(async () => {
        await withFakeDiff([{ type: "added", path: "rules/imp/.fake_scratch" }], async () => {
...
test("requires a non-empty outputPaths array", async () => {
    let thrown = null;
    try {
        await generatedFiles({ argv: ["true"], outputPaths: [], materialize: true });
    } catch (error) {
        thrown = error;
    }
